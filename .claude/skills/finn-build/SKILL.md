...
Using the Linear connector, list issues on team `ALI` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `ALI-NNN-short-slug`, using the issue's real identifier.
...
- `Closes ALI-NNN`, using the real Linear issue identifier
